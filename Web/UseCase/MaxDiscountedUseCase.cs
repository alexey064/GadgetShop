...
{
    public class MaxDiscountedUseCase : IUseCase<MaxDiscountedUseCase>
    {
        private readonly ShopContext DB;
        public MaxDiscountedUseCase(ShopContext db) 
        {
            DB = db;
        }
        public async Task<List<Product>> Execute(int skip, int count) 
        {
            List<Product> products = await DB.Products.Include(o => o.Accessory)
                ...
                .Where(o=>o.DiscountDate>System.DateTime.Now)
                .OrderByDescending(o => o.Discount).Skip(skip).Take(count).ToListAsync();

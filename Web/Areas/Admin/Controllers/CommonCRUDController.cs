...
namespace Web.Controllers
{
    public abstract class CommonCRUDController : Controller
    {
        ...
        public string LoadPhoto(IFormFile file, string filePath, string type)
        {
            ...
            using (FileStream fs = new FileStream(Directory.GetCurrentDirectory() + "/wwwroot/Files/"+type+"/"+type+"_" + (MaxNumb + 1) + ".png", FileMode.Create))
            {
                file.CopyTo(fs);
            }
            return "/Files/"+type+"/"+type+"_" + (MaxNumb + 1) + ".png";
        }
    }
}

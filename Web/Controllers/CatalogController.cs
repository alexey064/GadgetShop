...
namespace Web.Controllers
{
    public class CatalogController : Controller
    {
        ...
        public CatalogController(IProductRepo<Product> ProductRepository, ILinkedRepo<ProdMovement> ProdRepository,
        ...
        public async Task<IActionResult> Catalog(string type, int Page = 1)
        {
            ...
            switch (type)
            {
                case nameof(Accessory):
                    ...
                    output = await ProductRepo.GetByCategory((Page - 1) * itemPerPage, itemPerPage, nameof(Accessory)) as List<Product>;
                    break;
                case nameof(Notebook):
                    ViewBag.Type = nameof(Notebook);
                    count = await NotebookRepo.GetCount();
                    output = await ProductRepo.GetByCategory((Page - 1) * itemPerPage, itemPerPage, nameof(Notebook)) as List<Product>;
                    break;
                case nameof(Smartphone):
                    ViewBag.Type = nameof(Smartphone);
                    count = await SmartRepo.GetCount();
                    output = await ProductRepo.GetByCategory((Page - 1) * itemPerPage, itemPerPage, nameof(Smartphone)) as List<Product>;
                    break;
                case nameof(WireHeadphone):
                    ViewBag.Type = nameof(WireHeadphone);
                    count = await WireRepo.GetCount();
                    output = await ProductRepo.GetByCategory((Page - 1) * itemPerPage, itemPerPage, nameof(WireHeadphone)) as List<Product>;
                    break;
                case nameof(WirelessHeadphone):
                    ViewBag.Type = nameof(WirelessHeadphone);
                    count = await WirelessRepo.GetCount();
                    output = await ProductRepo.GetByCategory((Page - 1) * itemPerPage, itemPerPage, nameof(WirelessHeadphone)) as List<Product>;
                    break;
            }
